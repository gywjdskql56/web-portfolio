...
file_list = ['미래에셋 추천 포트폴리오(국내)','미래에셋 추천 포트폴리오(해외)','미래에셋 추천 포트폴리오(연금)']
## 추천포트폴리오 엑셀 변환
for file_nm in file_list:
    port1 = pd.read_excel('org-data/suggest/{}_33.xlsx'.format(file_nm), sheet_name='적극투자형')
    port2 = pd.read_excel('org-data/suggest/{}_33.xlsx'.format(file_nm), sheet_name='위험중립형')
    port3 = pd.read_excel('org-data/suggest/{}_33.xlsx'.format(file_nm), sheet_name='안정추구형')
